...
pc.plot.heatmap <- function(comparison_df, 
  ...
  heatmap_plot <- ggplot(comparison_df, aes(x = Method1, y = Method2, fill = Count)) +
    ...
    theme_minimal(base_size = 14) +  # aumenta base da fonte
    labs(title = title, x = "Method 1", y = "Method 2", fill = "Count") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 14),  # eixo x maior
          axis.text.y = element_text(size = 14))  # eixo y maior  
